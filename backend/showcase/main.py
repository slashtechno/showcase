...
def attend_event(join_code: Annotated[str, Query(description="A unique code used to join an event")], current_user: Annotated[dict, Depends(get_current_user)]):
    """
    ...
    if db.user.get_user_record_id_by_email(current_user['email']) in event["fields"].get('attendees', []):
        raise HTTPException(status_code=400, detail="User already attending event")
    events.update(event['id'], {
        "attendees": event["fields"].get('attendees', []) + [db.user.get_user_record_id_by_email(current_user['email'])]
    })
